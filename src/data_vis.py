...
def generate_course_grade_histogram(
    ...
    if scorecard_out_path:
        ...
        fig.savefig(scorecard_out_path, facecolor="#fdfdfd")
    plt.close(fig)
...
def generate_instructor_course_history_overlay_graph(
    ...
    Output file is written into scorecard_dir so it can be embedded in short cards.
    """
    paths = config.get("paths", {}) or config.get("PATHS", {})
    output_dir = paths.get("scorecard_dir")
    if not output_dir:
        raise KeyError("scorecard_dir not found in config['paths'].")
    ...
    def _course_level(catalog_value: Any) -> Optional[int]:
